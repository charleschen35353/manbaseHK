const individual_register_on_load = () => {
    ...
    $('#ind_reg_sec_1_next').click(() => {
        ...
        for (el of ['ind_register_account', 'ind_register_password', 'ind_register_confirm_password']) {
        ...
        if ($('#ind_register_account').val().length == 0) {
            ...
            $('#ind_register_account_error').append("<p class='error'>ACCOUNT_NOT_EMPTY_ERROR</p>");
        } else {
        ...
        if ($('#ind_register_password').val().length < 8) {
            ...
            $('#ind_register_password_error').append("<p class='error'>PASSWORD_POLICY_VIOLATED</p>");
        } else {
            $('#ind_register_password').addClass('is-valid');
        }

        // Repeat Password Verification
        if ($('#ind_register_confirm_password').val() != $('#ind_register_password').val()) {
            error = true;
            $('#ind_register_confirm_password').addClass('is-invalid');

            $('#ind_register_confirm_password_error').append("<p class='error'>REPEAT_PASSWORD_VERIFICATION_FAILED</p>");
        } else {
